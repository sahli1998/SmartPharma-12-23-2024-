...
namespace SmartPharma5.ViewModel
{
    class SammaryViewModel : BaseViewModel
    {
        ...
        private int unpaied_invoice;
        public int Unpaied_invoice { get => unpaied_invoice; set => SetProperty(ref unpaied_invoice, value); }


        private int unpaied_invoice_due;
        public int Unpaied_invoice_due { get => unpaied_invoice_due; set => SetProperty(ref unpaied_invoice_due, value); }
...

        {
            ...
            try
            {
                ...
                if (queryunpaid.AsEnumerable().Count() > 0)
                {
                    ...
                    Unpaied_invoice = (int)queryunpaid.AsEnumerable().Select(x => x.Field<decimal>("unpaied_invoice")).Sum();
                    Unpaied_invoice_due = (int)queryunpaid.AsEnumerable().Select(x => x.Field<decimal>("unpaied_invoice_due")).Sum();
                    //TotalUnpaid = (int)queryunpaid.AsEnumerable().Select(x => x.Field<decimal>("restAmount")).Sum();
                    try
                    {
        ...
        public DataTable GetDataUnpaid()
        {
            ...
            if (DbConnection.Connecter())
            {
                ...
                string sqlCmd = "SELECT sale_balance.piece_type, sale_balance.piece_typeName, sale_balance.Id, sale_balance.code, sale_balance.reference,\r\n\r\nsale_balance.`Date`, sale_balance.IdPartner, sale_balance.partnerName, sale_balance.payment_conditionName,\r\n\r\nsale_balance.payment_methodName,  sale_balance.total_amount, sale_balance.paied_amount,\r\n\r\nsale_balance.restAmount,\r\n\r\nif(piece_type = 'Sale.Invoice, Sale, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null' , restAmount, 0) as unpaied_invoice,\r\n\r\nif(piece_type = 'Sale.Invoice, Sale, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null'  and date(sale_balance.due_date) < now(), restAmount, 0) as unpaied_invoice_due,\r\n\r\nsale_balance.due_date, commercial_partner.reference AS PartnerRef, commercial_partner.email AS Email,\r\n\r\ncommercial_partner_category.name AS PartnerCategory,\r\n\r\nsale_balance.pieceAgent, sale_balance.partnerAgent,\r\n\r\nif(sale_balance.piece_type like '%invoice%', 1,\r\n\r\nif(sale_balance.piece_type like '%shipping%',2,3)) as rang\r\n\r\nFROM sale_balance\r\n\r\nLEFT OUTER JOIN commercial_partner ON sale_balance.IdPartner = commercial_partner.Id\r\n\r\nLEFT OUTER JOIN commercial_partner_category ON commercial_partner.category = commercial_partner_category.Id\r\n\r\nWHERE(FORMAT(sale_balance.restAmount, 3) <> 0) ORDER BY rang, due_date;";
        ...
        public DataTable GetDataUnpaidByAgent()
        {
            ...
            if (DbConnection.Connecter())
            {
                ...
                string sqlCmd = "SELECT sale_balance.piece_type, sale_balance.piece_typeName, sale_balance.Id, sale_balance.code, sale_balance.reference,\r\n\r\nsale_balance.`Date`, sale_balance.IdPartner, sale_balance.partnerName, sale_balance.payment_conditionName,\r\n\r\nsale_balance.payment_methodName,  sale_balance.total_amount, sale_balance.paied_amount,\r\n\r\nsale_balance.restAmount,\r\n\r\nif(piece_type = 'Sale.Invoice, Sale, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null' , restAmount, 0) as unpaied_invoice,\r\n\r\nif(piece_type = 'Sale.Invoice, Sale, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null'  and date(sale_balance.due_date) < now(), restAmount, 0) as unpaied_invoice_due,\r\n\r\nsale_balance.due_date, commercial_partner.reference AS PartnerRef, commercial_partner.email AS Email,\r\n\r\ncommercial_partner_category.name AS PartnerCategory,\r\n\r\nsale_balance.pieceAgent, sale_balance.partnerAgent,\r\n\r\nif(sale_balance.piece_type like '%invoice%', 1,\r\n\r\nif(sale_balance.piece_type like '%shipping%',2,3)) as rang\r\n\r\nFROM sale_balance\r\n\r\nLEFT OUTER JOIN commercial_partner ON sale_balance.IdPartner = commercial_partner.Id\r\n\r\nLEFT OUTER JOIN commercial_partner_category ON commercial_partner.category = commercial_partner_category.Id\r\n\r\nWHERE(FORMAT(sale_balance.restAmount, 3) <> 0) AND (commercial_partner.sale_agent=" + IdAgent + ") " + "ORDER BY rang, due_date;";
